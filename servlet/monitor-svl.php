...
$result=mysql_query("SELECT `user` FROM `User`; ", $conn);
while($one=mysql_fetch_array($result)){
    $result1=mysql_query("SELECT `status`,count(*) FROM `RecitingWord` WHERE `user`='".$one[0]."' group by `status`; ");
    ...
    $result1=mysql_query("SELECT `word` FROM `RecitingWord` WHERE `user`='".$one[0]."' ORDER BY `lastRecite` DESC LIMIT 0,1; ");
    $x=mysql_fetch_array($result1);
    print "<td><a href='http://www.merriam-webster.com/dictionary/".$x[0]."'>".$x[0]."</a></td>";
    
    $result1=mysql_query("SELECT `word`,`status`,`lastRecite` FROM `RecitingWord` WHERE status<10 and DATE_ADD(`lastRecite`, INTERVAL power(`status`,4) MINUTE)<now() and user='".$one[0]."' ORDER BY `RecitingWord`.`RWID` ASC LIMIT 0,1; ");
    $x=mysql_fetch_array($result1);
    print "<td><a href='http://www.merriam-webster.com/dictionary/".$x[0]."'>".$x[0]."</a></td>";
